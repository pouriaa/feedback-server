...
async function initLocalPrisma() {
  ...
  return new PrismaClient({
    log: false ? ["error", "warn"] : ["error"]
  });
}
async function initWorkersPrisma(d1Binding) {
  const { PrismaD1 } = await import("@prisma/adapter-d1");
  const adapter = new PrismaD1(d1Binding);
  return new PrismaClient({ adapter });
}
function setD1Binding(binding) {
...
var isWorkersEnvironment, globalForPrisma, prisma;
var init_client = __esm({
  "src/db/client.ts"() {
    "use strict";
    isWorkersEnvironment = typeof globalThis.caches !== "undefined" && typeof globalThis.caches.default !== "undefined";
...
__export(db_exports, {
  ...
  getPrisma: () => getPrisma,
  initializePrisma: () => initializePrisma,
  prisma: () => prisma,
  setD1Binding: () => setD1Binding,
...
var FeedbackService = class {
  ...
  async getCount(projectId) {
    const prisma2 = getPrisma();
    return prisma2.feedback.count({ where: { projectId } });
  }
  /**
  ...
  async clear(projectId) {
    const prisma2 = getPrisma();
    await prisma2.feedback.deleteMany({ where: { projectId } });
  }
  /**
    ...
    return {
      ...
      context: {
        ...
        trigger: {
          ...
          element: feedback.triggerElement ?? void 0,
          coordinates: feedback.triggerCoordX !== null && feedback.triggerCoordY !== null ? { x: feedback.triggerCoordX, y: feedback.triggerCoordY } : void 0
        }
      },
      response: {
...
var SnapshotService = class {
  ...
  async processSnapshot(data, projectId) {
    const prisma2 = getPrisma();
    const existingSnapshot = await prisma2.snapshot.findUnique({
      where: {
    ...
    if (!existingSnapshot) {
      console.log(
        `[SnapshotService] First snapshot for session ${data.sessionId} at ${data.url}`
      );
      return {
        hasChanges: false,
        changedElements: []
    ...
    if (!hasChanges) {
      console.log(
        `[SnapshotService] No changes detected for session ${data.sessionId} at ${data.url}`
      );
      return {
        hasChanges: false,
        changedElements: []
    ...
    console.log(
      `[SnapshotService] Changes detected for session ${data.sessionId} at ${data.url}`,
      {
        oldFingerprint: existingSnapshot.fingerprint.substring(0, 20) + "...",
        newFingerprint: data.fingerprint.substring(0, 20) + "..."
      }
    );
    const changedElements = this.detectChangedElements(
      existingSnapshot.html,
      data.html
    );
  ...
  generatePromptConfig(changedElements) {
    ...
    const hasAddedButtons = changedElements.some(
      (el) => el.changeType === "added" && (el.selector === "button" || el.selector === "[role='button']")
    );
    const hasContentChanges = changedElements.some(
      (el) => el.changeType === "modified" && el.metadata?.contentChanged
    );
    const hasFormChanges = changedElements.some(
      (el) => el.selector === "form" || el.selector === "input"
    );
    if (hasFormChanges) {
      return {
  ...
  async getById(id, projectId) {
    const prisma2 = getPrisma();
    const snapshot = await prisma2.snapshot.findUnique({
      where: { id }
    });
  ...
  async getBySessionId(sessionId, projectId) {
    const prisma2 = getPrisma();
    const snapshots = await prisma2.snapshot.findMany({
      where: { sessionId, projectId },
      orderBy: { receivedAt: "desc" }
    });
  ...
  async getCount(projectId) {
    const prisma2 = getPrisma();
    return prisma2.snapshot.count({ where: { projectId } });
  }
  /**
  ...
  async clear(projectId) {
    const prisma2 = getPrisma();
    await prisma2.snapshot.deleteMany({ where: { projectId } });
  }
  /**
...
// src/types/index.ts
var TriggerTypeSchema = external_exports.enum(["rageClick", "custom", "snapshot"]);
var PromptTypeSchema = external_exports.enum(["rating", "choice", "text"]);
...
var TriggerInfoSchema = external_exports.object({
  type: TriggerTypeSchema,
  element: external_exports.string().optional(),
  coordinates: CoordinatesSchema.optional()
});
var ContextDataSchema = external_exports.object({
  timestamp: external_exports.string().datetime({ message: "Must be a valid ISO 8601 timestamp" }),
  url: external_exports.string().url({ message: "Must be a valid URL" }),
  referrer: external_exports.string(),
  userAgent: external_exports.string(),
  viewport: ViewportSchema,
  sessionId: external_exports.string().uuid({ message: "Must be a valid UUID v4" }),
  trigger: TriggerInfoSchema,
  domPath: external_exports.string()
});
...
var FeedbackDataSchema = external_exports.object({
  context: ContextDataSchema,
  response: FeedbackResponseSchema
});
var FeedbackSubmissionResponseSchema = external_exports.object({
  success: external_exports.literal(true),
  id: external_exports.string()
});
var SnapshotDataSchema = external_exports.object({
  html: external_exports.string().min(1, { message: "HTML content is required" }),
  url: external_exports.string().url({ message: "Must be a valid URL" }),
  timestamp: external_exports.string().datetime({ message: "Must be a valid ISO 8601 timestamp" }),
  sessionId: external_exports.string().uuid({ message: "Must be a valid UUID v4" }),
  fingerprint: external_exports.string().min(1, { message: "Fingerprint is required" }),
...
var ChangedElementSchema = external_exports.object({
  selector: external_exports.string(),
  changeType: ChangeTypeSchema,
  metadata: external_exports.record(external_exports.unknown()).optional()
});
var ChangeDetectionResponseSchema = external_exports.object({
  hasChanges: external_exports.boolean(),
  changedElements: external_exports.array(ChangedElementSchema),
...
var HealthCheckResponseSchema = external_exports.object({
...
async function validateApiKey(apiKey, origin) {
  ...
  const { getPrisma: getPrisma2 } = await Promise.resolve().then(() => (init_db(), db_exports));
  const prisma2 = getPrisma2();
...
function validateAdminKey(adminKey, expectedKey) {
...
app.post("/projects", async (c) => {
  ...
  try {
    const body = await c.req.json();
    const { name, allowedOrigins } = body;
    if (!name) {
...
app.patch("/projects/:id", async (c) => {
  ...
  try {
    ...
    const { getPrisma: getPrisma2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const prisma2 = getPrisma2();
    const project = await prisma2.project.update({
      where: { id: c.req.param("id") },
      data: { name },
      include: { allowedOrigins: true }
    });
...
app.post("/projects/:id/origins", async (c) => {
  ...
  try {
    ...
    const { getPrisma: getPrisma2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const prisma2 = getPrisma2();
    const allowedOrigin = await prisma2.allowedOrigin.create({
      data: {
...
app.delete("/projects/:id/origins/:originId", async (c) => {
  ...
  try {
    const { getPrisma: getPrisma2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const prisma2 = getPrisma2();
    await prisma2.allowedOrigin.delete({
